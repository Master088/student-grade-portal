...
// fires when the register button is click
if (isset($_POST['submit'])) {
    // get data from post request and set it to variable
    $lrn = $_POST['lrn'];
    ...
    // if password and confirm is not match
    if ($password != $password2) {
        echo '<script>';
        echo "alert('password not macth!');";
        echo '</script>';
    } else {
        // set new name for file(image profile)
        $date = date_create();
        ...
        if (move_uploaded_file($temp, $directory)) {
            ...
            // check if the lrn is already taken 
            $sql = "SELECT * FROM  student where lrn='$lrn'";
            $res = mysqli_query($conn, $sql);
            // yes display alert error
